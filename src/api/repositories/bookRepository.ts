...
export class BookRepository {
  ...
  async getPopularBooks(limit: number = 10): Promise<any[]> {
    const result = await Book.sequelize?.query(`
      SELECT 
        b.id,
        b.title,
        b.author,
   ...
   */
  async getMostReviewedAuthors(limit: number = 5): Promise<any[]> {
    const result = await Book.sequelize?.query(`
      SELECT 
        b.author,
   ...
   */
  async getActiveUsers(limit: number = 20): Promise<any[]> {
    const result = await Book.sequelize?.query(`
      SELECT 
        u.id,
        u.name,
   ...
   */
  async getSuccessfulTrades(): Promise<any[]> {
    const result = await Book.sequelize?.query(`
      SELECT 
        bt.id as trade_id,
   ...
   */
  async getGenreStatistics(): Promise<any[]> {
    const result = await Book.sequelize?.query(`
      SELECT 
        genre,
   ...
   */
  async getTradeAvailableBooks(): Promise<any[]> {
    const result = await Book.sequelize?.query(`
      SELECT 
        b.id,
        b.title,
   ...
   */
  async getRecommendedBooks(genreFilter?: string, limit: number = 10): Promise<Book[]> {
    ...
    if (genreFilter) replacements.genre = genreFilter;

    const results = await Book.sequelize?.query(query, {
      replacements,
      type: QueryTypes.SELECT,
      mapToModel: true,
      model: Book
